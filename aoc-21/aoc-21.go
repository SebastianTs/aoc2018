...
func main() {
	...
	for i:=cur[ip]; i < len(program); {
		...
		i = cur[ip]+1
		if i == 29 { // eqrr 5 0 1
			//fmt.Println(i, p.Code, p.A, p.B, p.C, "\t", cur)
			fmt.Println("The value for register 0 to halt the program with the fewest instructions is", cur[myPuzzleRegister])
			break
		}
	}
